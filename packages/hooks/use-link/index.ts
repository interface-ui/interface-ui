...
export const useLink = (props: LlinkProps, ns: UseNamespaceReturn) => {
  ...
  const styles = {
    [ns.b()]: {
      '--fn-link__underline--status': props.underline === 'none' ? 'none' : props.underline === 'hover' ? 'none' : 'underline',
      '--fn-link__underline--hover': props.underline === 'none' ? 'none' : props.underline === 'hover' ? 'underline' : 'underline',
